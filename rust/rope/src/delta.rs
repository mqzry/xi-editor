...
impl<N: NodeInfo> Delta<N> {
    ...
    pub fn apply_in_place(&self, base: &mut Node<N>) {
        debug_assert_eq!(
            base.len(),
            self.base_len,
            "must apply Delta to Node of correct length"
        );
        if self.try_apply(base) {
            return;
        }
...
mod tests {
    ...
    fn simple_in_place() {
        let d = Delta::simple_edit(Interval::new_closed_open(1, 9), Rope::from("era"), 11);
        assert_eq!("herald", d.apply_to_string_in_place("hello world"));
        assert_eq!(6, d.new_document_len());
    }
    ...
    fn factor_in_place() {
        let d = Delta::simple_edit(Interval::new_closed_open(1, 9), Rope::from("era"), 11);
        let (d1, ss) = d.factor();
        assert_eq!("heraello world", d1.apply_to_string_in_place("hello world"));
        assert_eq!("hld", ss.delete_from_string("hello world"));
    }
